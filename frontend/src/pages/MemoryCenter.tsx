...
export default function MemoryCenter() {
  ...
  const load = async () => {
    try {
      const result = await api.memories(status || undefined);
  ...
  const draft = async () => {
    ...
    setError(null);
    try {
      await api.draftMemory({ ...form, source: "web_ui" });
      setForm({ scope: "house", owner: "", subject: "", key: "", value: "" });
      await load();
    } catch (e: any) {
      setError(e.message || String(e));
    } finally {
  ...
  return (
    <div>
      <PageHeader
        title="Memory Center"
        subtitle="Approval-first house memory for preferences, device quirks, and corrections"
