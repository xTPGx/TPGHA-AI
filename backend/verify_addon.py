...
def main() -> int:
    ...
    check("Discovery smart domain automapper is wired",
          '"light": "lights"' in discovery_classifier
          and '"fan": "fans"' in discovery_classifier
          and '"person": "person"' in discovery_classifier
          and "personal_device_sensor" in discovery_classifier
          and "_merge_room_overlay" in config_loader
          and "_upsert_room_member" in discovery_registry
    ...
    check("phase 115 setup onboarding path is wired",
          "Owner onboarding path" in (repo_root / "frontend" / "src" / "pages" / "Setup.tsx").read_text(encoding="utf-8")
          and "api.onboardingPlan" in (repo_root / "frontend" / "src" / "pages" / "Setup.tsx").read_text(encoding="utf-8")
          and "setup_onboarding_path" in (repo_root / "backend" / "app" / "brain.py").read_text(encoding="utf-8")
          and "build_jarvis_phase_115" in experience_brain,
          "Setup must surface the ordered onboarding path from the operations brain.")
    ...
    check("phase 147 release recommendation history is wired",
          "ReleaseRecommendationEvent" in (repo_root / "backend" / "app" / "db" / "models.py").read_text(encoding="utf-8")
          and "/release/status-history/recommendations/history" in backend_main
          and "list_release_recommendation_history" in experience_brain
          and "Recent recommendation history" in dashboard_frontend
          and "api.releaseRecommendationHistory" in dashboard_frontend
          and "release_recommendation_history" in (repo_root / "backend" / "app" / "brain.py").read_text(encoding="utf-8")
          and "build_jarvis_phase_147" in experience_brain,
          "Owners need a history trail for recommendation state changes.")
    ...
    check("phase 149 release packet bundle is wired",
          "/release/packet" in backend_main
          and "build_release_packet" in experience_brain
          and "Copy full release packet" in dashboard_frontend
          and "Download full release packet" in dashboard_frontend
          and "api.releasePacket" in dashboard_frontend
          and "release_packet_bundle" in (repo_root / "backend" / "app" / "brain.py").read_text(encoding="utf-8")
          and "build_jarvis_phase_149" in experience_brain,
          "Owners need a single full release packet with checklist, decisions, metrics, recommendations, and runbook.")
    ...
    r = client.get("/brain/phase-115")
    check("/brain/phase-115 returns JSON",
          r.status_code == 200 and is_json(r),
          f"status={r.status_code} ctype={r.headers.get('content-type')}")
    check("/brain/phase-115 has setup onboarding path marker",
    ...
    r = client.get("/release/status-history/recommendations/history")
    check("/release/status-history/recommendations/history returns event trail",
          r.status_code == 200
          and is_json(r)
          and r.json().get("count", 0) >= 2
          and any(event.get("recommendation_id") == "resolve_release_blockers" for event in r.json().get("events", []))
          and "# TPG HomeAI Release Recommendation History" in r.json().get("markdown", ""),
          str(r.json()))
    r = client.get("/brain/phase-147")
    check("/brain/phase-147 returns JSON",
          r.status_code == 200 and is_json(r),
          f"status={r.status_code} ctype={r.headers.get('content-type')}")
    ...
    check("/release/packet returns full release packet",
          r.status_code == 200
          and is_json(r)
          and "# TPG HomeAI Full Release Packet" in r.json().get("markdown", "")
          and "checklist" in r.json()
          and "decisions" in r.json()
          and "metrics" in r.json()
          and "recommendations" in r.json()
          and "runbook" in r.json(),
          str(r.json()))
    r = client.get("/brain/phase-149")
    check("/brain/phase-149 returns JSON",
          r.status_code == 200 and is_json(r),
          f"status={r.status_code} ctype={r.headers.get('content-type')}")
